from AppKit import NSView, NSMenuItem, NSAffineTransform, NSRectFill, NSColor, NSMakePoint
import vanilla
...
class DefconAppKitGlyphThemeNSView(NSView):
    ...
    def _setFrame(self):
        ...
        if scrollHeight > height:
            height = scrollHeight
        # set the frame
        self.setFrame_(((0, 0), (scrollWidth, scrollHeight)))
        self._fitToFrame = self.superview().visibleRect().size
        # calculate and store the vetical centering offset
        self._verticalCenterYBuffer = (height - glyphHeight) / 2.0
    ...
    def drawBlues(self, glyph, layerName):
        colorBlues = drawing.colorToNSColor(self._theme["glyphViewBluesColor"])
        backgroundColor = drawing.colorToNSColor(self._theme["glyphViewBackgroundColor"])
        drawing.drawFontPostscriptBlues(glyph, self._inverseScale, self._drawingRect, color=colorBlues, backgroundColor=self._backgroundColor)

    @python_method
    def drawFamilyBlues(self, glyph, layerName):
        colorFamilyBlues = drawing.colorToNSColor(self._theme["glyphViewFamilyBluesColor"])
        backgroundColor = drawing.colorToNSColor(self._theme["glyphViewBackgroundColor"])
        drawing.drawFontPostscriptFamilyBlues(glyph, self._inverseScale, self._drawingRect, color=colorFamilyBlues, backgroundColor=self._backgroundColor)
    ...
    def drawFillAndStroke(self, glyph, layerName):
        contourFillColor = drawing.colorToNSColor(self._theme["glyphViewAlternateFillColor"])
        contourStrokeColor = drawing.colorToNSColor(self._theme["glyphViewStrokeColor"])
        componentFillColor = drawing.colorToNSColor(self._theme["glyphViewComponentFillColor"])
        componentStrokeColor = drawing.colorToNSColor(self._theme["glyphViewComponentStrokeColor"])
        backgroundColor = drawing.colorToNSColor(self._theme["glyphViewBackgroundColor"])
        strokeWidth = self._theme["glyphViewStrokeWidth"]
    ...
    def drawPoints(self, glyph, layerName):
        colorCornerPointsFill = drawing.colorToNSColor(self._theme["glyphViewCornerPointsFill"])
        colorCornerPointsStroke = drawing.colorToNSColor(self._theme["glyphViewCornerPointsStroke"])
        colorTangentPointsFill = drawing.colorToNSColor(self._theme["glyphViewTangentPointsFill"])
        colorTangentPointsStroke = drawing.colorToNSColor(self._theme["glyphViewTangentPointsStroke"])
        ...
        drawing.drawGlyphPoints(
            ...
            colorTangentPointsFill=colorTangentPointsFill,
            colorTangentPointsStroke=colorTangentPointsStroke,
            colorCurvePointsFill=colorCurvePointsFill,
            colorCurvePointsStroke=colorCurvePointsStroke,
            colorOffCurvePointsFill=colorOffCurvePointsFill,
            colorOffCurvePointsStroke=colorOffCurvePointsStroke,
            colorOffCurveQuadPointsStroke=colorOffCurveQuadPointsStroke,
            colorSmoothPointStroke=colorSmoothPointStroke,
            colorStartPointsArrow=colorStartPointsArrow,
            colorOpenStartPointsArrow=colorOpenStartPointsArrow,
            colorPointCoordinate=colorPointCoordinate,
            colorPointCoordinateBackground=colorPointCoordinateBackground,
            colorHandlesStroke=colorHandlesStroke,
            colorHandlesQuadStroke=colorHandlesQuadStroke,
            backgroundColor=backgroundColor,
            pointSizeOnCurve=pointSizeOnCurve,
            pointSizeOffCurve=pointSizeOffCurve)

    @python_method
    def drawAnchors(self, glyph, layerName):
        color = drawing.colorToNSColor(self._theme["glyphViewAnchorColor"])
        textColor = drawing.colorToNSColor(self._theme["glyphViewAnchorTextColor"])
        backgroundColor = drawing.colorToNSColor(self._theme["glyphViewBackgroundColor"])
...
class GlyphView(PlacardScrollView):
    ...
    glyphViewClass = DefconAppKitGlyphThemeNSView
